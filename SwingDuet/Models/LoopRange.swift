...
/// ループ範囲の端。フェーズからのコマ数（基準側動画のフレーム）で持つので、フェーズ調整や基準の切り替えで
...
struct LoopRange: Hashable {
    ...
    mutating func move(_ bound: Bound, to time: Double, in sync: SyncEngine) {
        let frame = sync.referenceFrameDuration
        // 開始は終了の 1 コマ手前まで、終了は開始の 1 コマ後から
        let other = sync.commonTime(of: self[bound == .start ? .end : .start])
        let limit = bound == .start ? other - frame : other + frame
        var edge = sync.loopEdge(nearest: bound == .start ? min(time, limit) : max(time, limit))
        // 丸めの起点（最も近いフェーズ）が反対側の端と違うと、丸めで半コマまで食い込むことがある。そのときは 1 コマ退く
        let gap = bound == .start ? other - sync.commonTime(of: edge) : sync.commonTime(of: edge) - other
...
extension SyncEngine {
    ...
    func commonTime(of edge: LoopEdge) -> Double {
        min(max(commonTime(of: edge.phase) + Double(edge.frames) * referenceFrameDuration, 0), commonDuration)
    }

    /// time に最も近いフェーズから整数コマの位置にある端
    func loopEdge(nearest time: Double) -> LoopEdge {
        let t = min(max(time, 0), commonDuration)
        let phase = SwingPhase.allCases.min { abs(commonTime(of: $0) - t) < abs(commonTime(of: $1) - t) } ?? .address
        let frames = Int(((t - commonTime(of: phase)) / referenceFrameDuration).rounded())
    ...
    func commonRange(of loop: LoopRange) -> ClosedRange<Double> {
        let lower = commonTime(of: loop.start)
        return lower...max(commonTime(of: loop.end), lower + referenceFrameDuration)
    }
}
