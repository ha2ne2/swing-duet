...
//
// ビルド:  swiftc -O -o build/analyze-swing SwingDuet/Services/SwingAnalyzer.swift SwingDuet/Models/SwingModels.swift scripts/analyze-swing/main.swift
...
func printSeries(_ result: SwingAnalysisResult) {
    let samples = SwingAnalyzer.speedSeries(track: result.wrists)
    let maxSpeed = samples.map(\.speed).max() ?? 1
    var si = 0
    for (t, p) in zip(result.wrists.times, result.wrists.points) {
        var speedText = ""
    ...
    for path in paths {
        ...
        do {
            ...
            print(String(format: "# %@  duration=%.2fs fps=%.1f 検出率=%.0f%%  (%.1fs)",
                         url.lastPathComponent, result.duration, result.frameRate, result.wrists.coverage * 100, elapsed))
