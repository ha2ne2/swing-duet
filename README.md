...
- 🎞️ **入力** — 画面は比較画面 1 枚。左右のペインの + からフォトライブラリの動画を選ぶ。右上の「＋ 新しいスイング」で自分の動画だけを差し替え（お手本はそのまま）。人物が収まるように自動で拡大し、ピンチで拡大縮小・ドラッグで位置合わせ（保存される）
...
- ▶️ **再生** — 共通シークバー（区間色分け）、フェーズジャンプ、0.1〜1.0 倍速、コマ送り、区間ループ。各動画の下の「フェーズ調整」で検出結果を直せる
