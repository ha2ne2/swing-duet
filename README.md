...
- 🎥 **撮影** — ホームの「撮影」で三脚に置いて録画を押し、打つだけ。240fps で撮りながらスイングを検出し、1 球ずつのスロー動画が写真ライブラリのアルバム「SwingDuet」と今日の一覧に残る（素振りは残さない）。打席に届く合図は音（構えた / 切れている / 取れた / 止まった。「…」で切れる）。熱・電池・容量・不在で自動で止まる
- 🎞️ **入力** — ホームの「ライブラリから」で写真ライブラリから 1 本選ぶと、そのスイングとお手本を並べたステージが開く。動画を選ぶ画面は「動画」（写真ライブラリの動画だけを撮影日順に。スロー撮影は原本のまま取り込む）と「お手本」の 2 タブ。人物が収まるように自動で拡大し、ピンチで拡大縮小・ドラッグで位置合わせ（保存される）
- 📓 **スイングの記録** — 取り込んだスイングは撮影日ごとに一覧に残る。★ を付けたものは「★ お気に入り」に固定され、それ以外は 200 本で流れる（今日の分は数えない）。削除は「元に戻す」で戻せる
...
- ▶️ **再生** — 共通シークバー（区間色分け）、フェーズジャンプ、再生速度（タップするたびに x1 → x1/2 → x1/4 → x1/8 → x1）、ジョグホイールでコマ送り（回した分だけ進み、回し続けると 1 周ごとに加速。1 コマごとに振動）、ループ範囲（区間のプリセットと、シークバー両端のつまみでコマ単位に広げ縮め）。各動画の下の「フェーズ調整」で検出結果を直せる
- 🧭 **関節の軌跡** — 手・頭・左右の肩・左右の股関節が通った道を動画に重ねる（右上のボタンでオン・オフ、隣の「…」で部位ごとの出し分け）。再生位置までに通ったところだけを描き、部位ごとに色を変え、トップより前は淡く以降は同系の濃い色にして行きと帰りを見分ける。肩と股関節は左右 2 本ずつなので体の回旋が読める
- 💾 **保存** — 解析結果・手直し・位置合わせ・最後に比べた相手はスイングごとに自動で残る。保存ボタンは無い。動画は写真ライブラリを参照するのでコピーで容量を食わず、アプリで消しても写真には残る

保存データを安全に読めない場合は、動画を保護してホームに保存停止を表示します。撮影の保存失敗時も作業動画を残します（[保存と撮影の仕組み](./docs/ARCHITECTURE.md)）。

MVP から除外: 線引き / 棒人間 / 動画書き出し / YouTube 連携（詳細は [docs/SPEC.md](./docs/SPEC.md)）

## 開発

コードの入口と責務の分担は [アーキテクチャのモジュール構成](docs/ARCHITECTURE.md#2-モジュール構成) を参照。
