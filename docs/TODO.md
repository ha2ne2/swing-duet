...
- **やること**:
  ...
  3. 手首の追跡（検出に使う側）にも同じ平滑化を掛けるか、窓幅をフレーム数ではなく秒で決めるかを一緒に決める。
     `SwingDetector.handSamples` の速度の移動平均も窓 5 フレーム固定で、解析（30fps）とライブ（15fps）で掛かる秒数が倍違う。
- **注意**: フィニッシュは山の頂点から `finishBand`（0.05）以内に入った点で決めるので、平滑化を変えたらフィニッシュの位置も見直す。
  検証に使う動画は worktree 間で共有されない（`docs/data/` は gitignore。[AGENTS.md](../AGENTS.md) §4.3）。
