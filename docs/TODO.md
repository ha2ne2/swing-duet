...
- [ ] H. 中継のスーパースローはトップ付近で追跡が落ち、焼き込みの速さも一定とは限らない（2026-09-11 起票）
...
- [ ] L. 関節の軌跡に残る誤検出（手の貼り付き・頭の跳び）（2026-09-14 起票）
...
- [ ] AC. ループ範囲のつまみを動かす間も再生を止めない（2026-09-15 起票）
- [ ] AD. `analyzingID` が「解析中」の表示と Vision の排他ロックを兼ねている（2026-09-15 起票）
- [ ] AE. フェーズ間の最低間隔が 2 か所で別の秒数（コマ数で持っていない）（2026-09-15 起票）
...
### H. 中継のスーパースローはトップ付近で追跡が落ち、焼き込みの速さも一定とは限らない（2026-09-11 起票 / 2026-09-14 に内容を差し替え）

- **背景**: 観客が並ぶ画で追跡が別人へ乗り換える件は解決済み（`PoseTracker.selectPerson` が候補を
  「胴体が縦向き」「大きさが前フレームから 25% 以内」に絞る）。
...
- **背景**: 撮影画面（[ARCHITECTURE.md](./ARCHITECTURE.md) §6）はシミュレータにカメラが無いので、実機でしか確かめられない。
- **対象**: `SwingDuet/Services/Capture/*`、`SwingDuet/Views/Capture/CaptureView.swift`
- **実機で通ったこと**: 1080p240 の `AVCaptureDevice.Format` の選択、`AVAssetWriter` へのコマ落ち 0（`SegmentWriter.droppedFrames`）、
  検出・切り出し・240fps の写真ライブラリへの保存（写真アプリでは実速で再生される）・帯・合図の音。
- **やること**:
  1. 止める直前に打席から戻る場面が 1 球になる。窓の先頭が前のスイングのアドレスを過ぎると、インパクトの窪みをアドレス、
     カメラに近づいて体が大きく写ったところをフィニッシュと読む。`LiveShotJudge.observe` で前のフィニッシュより前にアドレスがある候補を捨てる
     （[research/260913_1204](./research/260913_1204-capture-stop-phantom-shot-and-short-take.md) §3.5）。
  2. 残りの実機確認：30 分続けたときの熱（`systemPressureState` の推移）と電池、キーフレーム間隔 15 / 30 / 60 の 1 ショットの大きさと戻る操作、
     前面カメラ（120fps・鏡像）の向き、合図の音の音量、構えの判定の余白（関節の外接矩形が端から 4%）、
     区切りをつなぐ書き出し（`AVMutableMovie` のサンプルコピー。Mac でしか確かめていない）。
- **調査用の取り出し方**: 「全体の動画も残す」（既定オン）と `Documents/CaptureLogs/` のログ。
  `xcrun devicectl device copy from --device <ID> --domain-type appDataContainer --domain-identifier com.ha2ne2.SwingDuet --source Documents/CaptureLogs --destination <dir>`
  （動画は `Documents/CaptureTakes` に区切りごと。`build/analyze-swing --shots` の後解析とライブの `decision` の範囲が一致するかを確かめる）
...
- **背景**: 描く曲線・部位ごとの平滑化・区間近似・遮蔽をまたぐつなぎは実装済み
  （調査は [research/260914_0311](./research/260914_0311-joint-trail-smoothing.md)、
  [実測](./research/260915_0527-body-trail-fitting.md)・[実測](./research/260915_0624-hand-trail-occlusion.md)）。
  残るのは、平滑化では消せない誤検出そのもの。
- **対象**: `SwingDuet/Models/JointTrail.swift`、`SwingDuet/Services/Analysis/PoseTracker.swift`（`headLocation`）
- **やること**:
  1. **手が静止したままの誤検出を落とす。** インパクトを挟む 5 コマで Vision が手首を太ももに貼り付けた例がある
     （信頼度 0.32〜0.45 で、信頼度では落とせない）。手はトップ〜インパクトで最も速いので、
     その間のほぼ静止は採らない、という制約が効くはず。`JointTrails` にフェーズを渡す設計から要る。
     手の動きの向き（後方視点なら右下→左上、正面ならインパクト後に下→右→上→左）を使う案もあるが、
     視点と利き手の判定が先に要るうえ、貼り付き誤検出は向きでは落ちない。速さの制約を先に試す。
  2. 頭は鼻 → 目 → 耳の代用が切り替わると軌跡が跳ぶ。正面の動画でバックスイング中に体 2.0 個分動いた例があり、
     区間近似では直らない（[実測](./research/260915_0527-body-trail-fitting.md) §6）。
  3. それでも足りなければ Whittaker（罰則付き）平滑化に置き換える案がある（研究 §7 案 B）。
     欠けたコマの補間と、Vision の信頼度による重み付けも同じ式で入る。
  4. 検出側の窓はまだコマ数固定。`SwingDetector.handSamples` の速度の移動平均は 5 コマ固定で、
     解析（30fps）と撮影中のライブ追跡（15fps）で掛かる秒数が倍違う。
- **実機で見ること**: 区間近似のフォロー〜フィニッシュの先端のずれ（[検証](./research/260915_0328-hand-fit-video-check.md)）。
- **制約**: 平滑化は表示だけに閉じる。検出に使う手首の系列に掛けると macroy_behind が検出失敗になる（研究 §8）。
...
- **L との関係**: 触る場所は同じ。L は手が止まったままの貼り付き、N は見失いの前後の大きな飛び。
  どちらも「あり得ない動きの値を採らない」なので、設計は一緒に考える。
...
- **やらない理由（今）**: 手元の本数ではスクロールが引っかかっていない。ただし一覧が格子になって 1 画面の枚数が 3 倍近くになったので、
  実機でスクロールしてみて引っかかるなら先に入れる。
...
- **やらない理由（今）**: UI の判断が要る。一覧の削除は格子の長押しのメニューに移った（確認なしのまま）ので、3 通りの違いは残っている。
...
- **背景**: シークバーの両端のつまみはループの範囲を決めるもので、再生位置を決めるものではない。
  今はつまみをつかむと再生が止まり（`beginDrag`）、動かすたびにその端のコマを静止画で映す（`trim` の `show`）。
  実機で触ると、範囲を広げ縮めしている間ずっと絵が止まるのが邪魔になる。
  [design/260912_0252](./design/260912_0252-loop-trim-handles.md) の「ドラッグ中は端のコマを映す」という判断を取り消す。
- **対象**: `SwingDuet/Services/Playback/PlaybackController.swift`（`trim` / `beginDrag` / `endDrag` / `stepFrame`）、
  `SwingDuet/Views/Stage/SeekBarView.swift`
- **やること**:
  1. つまみのドラッグ中は再生を止めず、ループ再生を続ける。変えるのは範囲の端だけで、つまみの位置へはシークしない。
     次の折り返しから新しい範囲で回る（なぞり `scrub` は今までどおり、つかんだ位置へ動かす）。
  2. ジョグホイールのシークをループ範囲で止めない（`stepFrame` の `clampedToLoop`）。範囲の外へも送れて、
     再生もそこから普通に始まる。ループの範囲は**どこで折り返すか**だけを決める。
- **決めること**: 再生位置が範囲の外にあるときの折り返し方。今は `loop` の didSet が範囲外を見つけると先頭へ動かしている。
  「範囲へ入るまでは流し、入ったらそこから回る」か「範囲の終わりを越えたら先頭へ戻す」かを決める。

### AD. `analyzingID` が「解析中」の表示と Vision の排他ロックを兼ねている（2026-09-15 起票）

- **背景**: `ClipStore.analyzingID` は一覧とステージが「解析中…」を出すために読む `@Published` だが、
  同時に「Vision を同時に 1 本しか走らせない」ための鍵でもある（`processQueue` の `analyzingID == nil`）。
  軌跡だけの作り直し（`buildTrails`）でもこの値を立てるので、本来なら解析していないクリップに「解析中」と出るはずだが、
  **軌跡の作り直しは解析済みのクリップにしか起きないから表示に出ない**という偶然で成立している。
  解析待ちのクリップに軌跡を作らせる経路ができた日に、誤表示になる。
- **対象**: `SwingDuet/Services/Library/ClipStore.swift`（`analyzingID` / `processQueue` / `analyze` / `buildTrails`）
- **やること**: 鍵を別に持つ（`isRunning` のような観測しない変数）か、`analyzingID` を「解析中のクリップ」だけに限り、
  軌跡の作り直しは別の変数で示す。表示の条件（`analyzingID == clip.id` かつ解析待ち）は変えない。
- **やらない理由（今）**: 誤表示は今の経路では起きない。触ると解析キューの順序に関わるので、単体テストを足してから。

### AE. フェーズ間の最低間隔が 2 か所で別の秒数（コマ数で持っていない）（2026-09-15 起票）

- **背景**: フェーズの順序（アドレス < トップ < インパクト < フィニッシュ）を保つ最低間隔が
  `PhaseSet.assign`（手で動かすとき）は 0.02 秒、`PhaseSet.sanitize`（検出結果を整えるとき）は 0.05 秒と別々で、
  どちらも秒の直値。`PhaseSet` はフレームレートを知らないので「1 コマ以上空ける」という本来の意味を表せていない。
  `assign` のコメントの「約 1 コマ」は 30fps 前提で、240fps の原本では約 5 コマ分になる。
- **対象**: `SwingDuet/Models/Swing.swift`（`PhaseSet.assign` / `sanitize`）
- **やること**: 間隔を 1 つの定数に寄せる。可能ならコマ数で受け取る（呼び手は `VideoConfig.frameDuration` を持っている）。
- **やらない理由（今）**: 実害は出ていない（240fps でも 0.02 秒は 5 コマで、フェーズをそこまで詰める場面が無い）。
  `assign` は `LoopRange` と同じく「フェーズからのコマ数」で持つのが筋なので、AC（つまみ）を触るときに一緒に見る。
- **関連**: 同じ型の重複が U（振り切り度の式が後解析と撮影中で別）。
