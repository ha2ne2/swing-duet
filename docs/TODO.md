...
- [ ] M. フィニッシュが振り切る前の位置で止まる（正面視点、2026-09-14 起票）
...
- **背景**: 比較画面の軌跡表示で、新しく足した正面・広め（引き）の動画を見ると、スイング中に細かい振動が乗る。
  今の平滑化は解析時の 5 点 Savitzky–Golay 1 回（`JointTrails.smoothed()`）だけで、1 コマごとの揺れは半分になるが小刻みなブレは残る。
  手首の追跡側は 3 点メディアン（`PoseTracker.medianFilteredWrists`）のみ。
- **対象**: `SwingDuet/Models/JointTrail.swift`（`smoothed()`・`BodyPart`・`JointTrailSample`・保存形式の版）、
  `SwingDuet/Services/PoseTracker.swift`（`jointTrails` の組み立て、頭の代用と `medianFilteredWrists`）、
  `SwingDuet/Views/Shared/BodyPart+Color.swift`、`SwingDuet/Views/Stage/JointTrailOverlay.swift`
- **やること**:
  1. 前向き・後ろ向きの 2 パス（往復）で平滑化し、位相の遅れを出さずに振動だけ落とす。**手だけでなく全ての部位の軌跡**に掛ける。
  2. **頭の軌跡は出さない**（不要と判断）。`BodyPart.head` と鼻 → 目 → 耳の代用、色の割り当てを消す。保存形式の版を上げて既存の軌跡は作り直させる。
  3. 手首の追跡（検出に使う側）にも同じ平滑化を掛けるか、窓幅を fps に依存させるか（30fps と 240fps で同じ秒数になるように）、
     撮影中のライブ追跡（15fps・窓が短い）でも足りるかを決める。
- **注意**: 検証に使う動画は worktree 間で共有されない（`docs/data/` は gitignore。[AGENTS.md](../AGENTS.md) §4.3）。
  `build/analyze-swing --series --joints` で掛ける前後を比べる。

### M. フィニッシュが振り切る前の位置で止まる（正面視点、2026-09-14 起票）

- **背景**: フィニッシュは「**最後に振り切ったところ**」であってほしいが、正面から撮った動画ではその手前（振り切る途中）で止まる。
  今は「フォローで手が上がりきる山の高さの 90%（`finishHeightRatio`）に**最初に**達したところ」をフィニッシュにしている
  （`SwingDetector.swingCandidate`）。正面視点は手が体の前を横切って高さの上がり方が緩いので、
  山の途中で 90% に触れた時点を拾ってしまうと見られる。
- **対象**: `SwingDuet/Services/SwingDetector.swift`（`finishHeightRatio` / `finishDrop` とフィニッシュの決め方）
- **やること**: 「最初に比に達した点」ではなく、振り切って**手が止まる**（速度が落ちきる／山の頂点に達する）ところを取る案を実データで確かめる。
  視点（正面 / 後方）で挙動が違うので、後方視点の既存サンプルで退化しないことが条件。L の平滑化を入れると山の形が変わるので、L の後に着手する。
- **参照**: D（欠測に掛かるトップ・インパクトのずれ）と同じ `swingCandidate` を触るので、直すときは合わせて見る
