...
- **残っていること**:
  - 6 部位すべての全 3 区間に 6 制御点の近似とオン・オフを実装（2026-09-15）。保存形式・検出は変更しない。
    肩・股関節の方が効きが大きい（[実測](./research/260915_0527-body-trail-fitting.md)）。
    近似が測定点から離れすぎる区間は通常の描画に戻す。
    フォロー〜フィニッシュの先端のずれは引き続き実機で確認する（[検証](./research/260915_0328-hand-fit-video-check.md)）。
  - 頭は鼻 → 目 → 耳の代用が切り替わると軌跡が跳ぶ。正面の動画でバックスイング中に体 2.0 個分動いた例があり、
    近似では直らない（`PoseTracker.headLocation`。[実測](./research/260915_0527-body-trail-fitting.md) §6）。
  - 後方視点で手が体に隠れる間の誤検出と、数コマの遮蔽での線の途切れを直した（2026-09-15）。
    孤立した 1 コマだけの観測を使わず、欠測をまたぐつなぎは前後の速さと釣り合うときだけ行う。
    いまの位置はコマの間を補間し、隠れている間も線が一定の速さで伸びる
    （[実測](./research/260915_0624-hand-trail-occlusion.md)）。表示だけの変更で軌跡の版は上げていない。
  - **手が静止したままの誤検出を落とす。** インパクトを挟む 5 コマで Vision が手首を太ももに貼り付けた例がある
    （信頼度 0.32〜0.45 で、信頼度では落とせない）。手はトップ〜インパクトで最も速いので、
    その間のほぼ静止は採らない、という制約が効くはず。`JointTrails` にフェーズを渡す設計から要る
    （[実測](./research/260915_0624-hand-trail-occlusion.md) 末尾）。
    スイングの手の動きの前提（後方視点なら右下→左上、正面ならインパクト後に下→右→上→左）を使う案もあるが、
