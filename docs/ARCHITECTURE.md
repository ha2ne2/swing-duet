...
- **描く**（`JointTrailOverlay`）: スイング区間のうち**再生位置までに通ったところだけ**を描き、いまの位置に丸を打つ
  ...
  線を切る条件は `JointTrails.connects`：0.3 秒より長い欠けは切る。1 コマぶんの移動（0.25）までは素通し。
  欠測をまたいだ大きい移動は、**またいだ区間の速さが前後で見えていた速さと釣り合うときだけ**つなぐ（3 倍まで。距離も 2 倍で頭打ち）。
  後方視点のフォローでは手が数コマ体に隠れるので、その間もつないで線を伸ばしたい。一方、別の場所に貼り付いた誤検出へつなぐと
  止まっている点へ画面を横切る線が伸びる（実測は [research/260915_0624](./research/260915_0624-hand-trail-occlusion.md)）。
  前後のコマがどちらも欠測している「ぽつんと 1 コマだけの観測」は、線にも先端の丸にも使わない。
  いまの位置（`JointTrails.position(on:at:)`）は線の上をコマの間まで補間して返す。止めると、隠れている間は線が伸びず、
  見えた瞬間に一気に伸びて速さが不自然になる。線が切れているところでは、近い方の線の端（0.1 秒以内）に丸を置く。
...
- **軌跡の近似**（`TrailFit`）: 6 部位すべてについて、アドレス〜トップ、トップ〜インパクト、インパクト〜フィニッシュの
  各区間に、保存点から 6 制御点の 3 次 B スプラインを当てはめる。外れ値の影響を 8 回の再重み付けで抑え、端点は固定して隣の区間につなぐ。
  線と先端の丸を同じ近似から求め、線は固定した 241 点から再生位置までを表示する。
  通常の 80 点への間引きや描画時の B スプラインは重ねて掛けない。
  長い欠測・飛びで区切られた線を別々に近似し、12 点未満・胴体の観測なし・計算不能なら通常の描画へ戻す。
  当てはめた曲線が測定点から離れすぎていたら（95 パーセンタイルで軌跡自体の大きさの 25% 超）近似を捨てて通常の描画に戻す。
  測定がその軌跡の形に対して粗いと 6 個の制御点では形を追えず、輪が潰れて別の形の線になるため。
  効きは手より肩・股関節の方が大きい。動く量が小さいぶん、同じ距離を進む間に手の 2.6〜5 倍曲がっているので、
  曲がりが 4 割減り、測定点からの離れは軌跡の大きさの 2〜4% に収まる
  （実測は [research/260915_0527](./research/260915_0527-body-trail-fitting.md)）。
  保存点・フェーズ・映像の縦横比が変わったときだけバックグラウンドで計算し、表示用にキャッシュする。
  フェーズを手で直したときも再計算するが、保存データとフェーズ検出の入力は変えないので軌跡の版は上げない。
- **切り替え**: ステージ右上のボタン（`UserDefaults` の `showJointTrails`。アプリ全体で 1 つ。既定はオフ）。
  ...
  同じメニューの「軌跡を滑らかにする」で近似を切り替える（`UserDefaults` のキーは手だけに掛けていた頃の `smoothHandTrails` のまま。既定はオン）。
  設定は左右共通で再起動後も保持し、オフでは分割・丸の位置も含めて従来の描画へ戻す。
