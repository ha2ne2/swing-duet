...
- **音声トラックは落とす**：コピーは取り込み時にファイルを書き換え（`stripAudioTrack`）、写真ライブラリの参照はファイルを触れないので
  ...
  ドリフト補正がそれをシークで直し、そのシークがまた時計を止めて連鎖する（速度 × 停止時間 > 80ms で発生。実機のスロー再生で顕在化した。
...
1. **追跡**（PoseTracker）: `AVAssetReader` でフレームを読み、`frameRate / 30` 間隔で間引いて `VNDetectHumanBodyPoseRequest` を実行
   ...
   複数人が写るとき（Golfboy の 2 視点合成、観客の入る中継映像など）は、まず候補を 2 つの条件で絞る（`selectPerson`）:
   **胴体が縦向き**（腰と首の横の差 × 画面の縦横比 < 縦の差）と、追跡中なら**大きさが前フレームから 25% 以内**。
   観客が並ぶ画では Vision が別々の人の関節をつないだ骨格を返し、首〜腰が長いので「最も大きく写る人物」に勝ってしまう。
   また追跡中のゴルファー（首〜腰 0.15）が一瞬とれなくなった隙に、腰の近さだけで選ぶと後ろの観客（0.09）へ乗り換えてしまう
   （どちらもマキロイの中継動画で実測）。絞った候補から、初回は最も大きく写る人物、以降は腰の位置が前フレームに最も近い人物を選ぶ。
   合う候補が無いフレームは誰も選ばず、見失いとして本人が戻るのを待つ。
   追跡中の人物を 1 秒（解析フレーム 30 個）見失ったらアンカーを捨てて選び直す。
...
3. **スイングの読み取り**（`detect` / `swingCandidate`）: 手が低い（h < 0.3）区間から順に「低い → 高い（h ≥ 0.5）→ 低い → 高い」の形を読む。
   ...
   - フィニッシュ: フォローで手が上がりきる山（そこから 0.3 下がるまで）の高さに 0.05 まで近づいた最初の点（振り切った位置に着いた瞬間）
...
- **作る**（解析時に 1 回）: `SwingAnalysisResult.jointTrails` が `PoseTracker` の結果から、採用スイングの前後 1 秒
  ...
  （[research/260914_0311](./research/260914_0311-joint-trail-smoothing.md) §12）
...
- **描く**（`JointTrailOverlay`）: スイング区間のうち**再生位置までに通ったところだけ**を描き、いまの位置に丸を打つ
  （まだ通っていない先を薄く出すと、6 本ぶんの線が重なって形が読めない）。
  点は**通らず、近くを通る曲線**（一様 3 次 B スプライン）でつなぐ。点を必ず通る曲線では姿勢推定のブレがそのまま角として出るが、
  スイングは連続した運動なので通す必要がない（線の曲がりの揺れが半分になる。実測は
  [research/260914_0311](./research/260914_0311-joint-trail-smoothing.md) §7）。端だけは制御点を重ねて点に届かせる。
  0.3 秒より長い欠けと、隣のコマから 0.25 を超える飛びで線を切る（`JointTrails.strokes`）。
...
- **切り替え**: ステージ右上のボタン（`UserDefaults` の `showJointTrails`。アプリ全体で 1 つ。既定はオフ）。
  出しているときだけ隣に「…」が出て、部位を 4 つ（手・頭・肩・腰。左右の対はまとめる）で出し分けられる
  （`TrailPartGroup`。隠している組を `hiddenJointTrailParts` に整数 1 つで持ち、隠す側の桁を立てるので部位を足しても既定は表示）。
