...
1 本の動画に素振りなど複数のスイングが写っている前提で、アドレスごとに候補を作って採点する。
`SwingAnalyzer.analyze` が入口で、1 は `PoseTracker`、2〜5 は `SwingDetector`（Vision に依存しない純粋計算）が担う。
設計の経緯と 4 本の実測値は [design/260910_0236](./design/260910_0236-hand-height-phase-detection.md)。
...
3. **アドレス**（`addressRuns`）: h < 0.3（低い）かつ速度 < 0.7（静止）が 0.15 秒以上続く区間。その終わり（動き出す直前）がアドレスで、候補は区間ごとに 1 つ
4. **各候補のフェーズ**（`swingCandidate`）: 「低い → 高い（h ≥ 0.5）→ 低い → 高い」の形を読む
   - トップ: 手が最も高い時刻。ただしそこから 0.2 秒以上、下り始め（高さが下がって速度 ≥ 1.0）が来なければトップで止まっているので、下り始めをトップにする
   - インパクト: トップの後、次に高くなるまでの最低点
   - フィニッシュ: インパクトの後、高くて速度 < 1.5 が 0.2 秒続く最初の時刻（+0.2 秒）。無ければ次のアドレスの手前
   - 高くならない（振り上げただけ）・低いまま終わる（トップからアドレスへ戻すリハーサル、フィニッシュを取らない素振り）は候補にしない
...
5. **採点と採用**: `0.5 × 振り上げ（h の最大 − アドレスの h、候補内の最大で正規化）+ 0.5 × ピーク速度（同）+ 0.03 × 時系列順`。
   素振りは振り上げ・速度とも小さく、本番と同じ振り切りなら後のスイングが選ばれる。候補はすべて `VideoConfig.candidates` に保存し、
   フェーズ調整画面で切り替えられる
6. `PhaseSet.sanitize` で順序と範囲を強制

しきい値はすべて体の大きさ単位の絶対値（最大速度に対する相対値は使わない。手首の誤検出による偽の速度ピークで動かないため）。
